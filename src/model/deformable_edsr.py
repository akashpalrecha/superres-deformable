...
from deformable_kernels.modules import DeformKernelConv2d
...
def make_model(args, parent=False):
    return DeformableEDSR(args)

class DeformableEDSR(nn.Module):
    def __init__(self, args, conv=common.default_conv, dkconv=deformable_conv):
        super(DeformableEDSR, self).__init__()
        ...
        scale = args.scale[0]
        act = nn.ReLU(True)
        url_name = 'deformable_r{}f{}x{}'.format(n_resblocks, n_feats, scale)
        if url_name in url:
